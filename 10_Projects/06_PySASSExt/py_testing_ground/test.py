...
func = TT_Func("SImm", {"SImm", "UImm"}, func_arg, False, False, a)
dx = pickle.dumps(func)
dr = pickle.loads(dx)
print(func, dr)

reg = TT_Reg("reg_name", default, {}, TT_Alias("alias"))
dx = pickle.dumps(reg)
dr = pickle.loads(dx)
print(reg, dr)
...
ext = TT_Ext(a, reg, False)
dx = pickle.dumps(ext)
dr = pickle.loads(dx)
print(ext, dr)

param = TT_Param(a, [op1], reg, [ext], False, False)
dx = pickle.dumps(param)
dr = pickle.loads(dx)
print(param, dr)

cash = TT_Cash([c1, c2, param, c3], False)
