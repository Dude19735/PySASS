...
from py_sass import TT_Reg, TT_List, TT_Param
...
def print_class(sass:SM_SASS, class_name:str, lines:list, ljust=20):
    ...
    ext_values = dict(itt.chain.from_iterable([[(x[0],x[2]), (x[1],x[2])] for x in [(i[0],i[1], set(itt.chain.from_iterable(getattr(sass.sm.details.REGISTERS, i[0]).values()))) for i in opcode_ext_names + regs_ext_names]]))

    ext_vals = dict()
    zero_bits = sass.sm.details.FUNIT.encoding_width*[0]
    for i in sorted(class_.ENCODING, key=lambda x: sortby_encs(x['alias'])):
        ...
        if expr.startswith_alias():
            alias = str(expr.get_first_value().alias)
...
if __name__ == '__main__':
    ...
    for sm in sms:
        ...
        multiples = [i for i in sorted([(len(i[1]), i[0], i[1]) for i in itt.chain.from_iterable([[(bin_code,class_names) for bin_code,class_names in v.items() if len(class_names) > 1] for k,v in sass.sm.opcode_ref.items()])], key=lambda x: x[0], reverse=True)]
                        ...
                        if len(g) > 1:
                            ...
                            for c in g:
                                with open(p.format(sm, c), 'w') as ccc:
                                    ccc.write(str(sass.sm.classes_dict[c]))
                                if sass.sm.classes_dict[c].IS_ALTERNATE:
                                    alts.append(c)
                                else:
