...
class BInstr_dadd_B(BInstrBase):
    """This one tests the DADD instruction with barriers.
    ...
    def __init__(self, kk_sm:KK_SM, props:KernelWLoopControlProps):
        ...
        move = sc.SASS_KK__MOVImm(kk_sm, 
                                   exec_pred_inv=False, exec_pred=PT, 
                                   target_reg=R30, imm_val=0, 
                                   usched_info_reg=wait15)
        self.add_post_clock_prequels(move.class_name, move.enc_vals)
        move = sc.SASS_KK__MOVImm(kk_sm, 
                                   exec_pred_inv=False, exec_pred=PT, 
        ...
        main = sc.SASS_KK__dadd__RRR_RR(kk_sm,
                                         Pg_negate=False, Pg=PT,
                                         Rd=R30,
                                         Ra_reuse=False, Ra_negate=False, Ra_absolute=False, Ra=R32,
                                         Rc_reuse=False, Rc_negate=False, Rc_absolute=False, Rc=R34,
        ...
        # Inside the measured clock area, we move a default value to R30, which takes exactly 15 cycles. Subtract
        # them from props.cs2r_clk_reg_2 because that is the second clock measuring register. With this, we effectively
        # ignore the SASS_KK__MOVImm.
...
class BInstr_dfma_NB(BInstrBase):
    ...
    def __init__(self, kk_sm:KK_SM, props:KernelWLoopControlProps, wait_cycles:int):
        ...
        main = sc.SASS_KK__dadd__RRR_RR(kk_sm,
                                         ...
                                         Ra_reuse=False, Ra_negate=False, Ra_absolute=False, Ra=R32,
                                         Rc_reuse=False, Rc_negate=False, Rc_absolute=False, Rc=R34,
                                         usched_info_reg=main_usched_info, req=0, wr=0x0, rd=0x7
                                         )
        self.add_main(main.class_name, main.enc_vals)
...
class Benchmark_DADD(BenchmarkBase):
    def __init__(self, sm_nr:int, experiment_nr:int=0):
        super().__init__(sm_nr, 
                         name='dadd',
                         purpose="\n".join([
                            "These benchmarks test DADD with and without barrier.",
                            "* With barrier it takes 49 cycles",
                            "* Without barrier it takes 42 cycles",
                            "",
                            "Compared to FADD that takes 5 cycles, this is weird..."]),
        ...
        # [print(i) for i in BenchmarkBase.create_bins(4, [i for i in range(127)])]
        ...
        if experiment_nr in (0,1):
            ...
            bInstr = BInstr_dadd_B(self.kk_sm, props)
            exe_name_stem = '{0}/{1}'.format(self.modified_location, 'dadd__RRR_RR_B')
        ...
        if experiment_nr in (0,2):
            ...
            exe_name_stem = '{0}/{1}'.format(self.modified_location, 'dadd__RRR_RR_NB')
...
if __name__ == '__main__':
    b = Benchmark_DADD(75, experiment_nr=0)
    print("Finished")
    pass
