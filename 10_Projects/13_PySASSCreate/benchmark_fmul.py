...
class BInstr(BInstrBase):
    ...
    def __init__(self, kk_sm:KK_SM, props:KernelWLoopControlProps, wait_cycles:int):
        ...
        main = sc.SASS_KK__fmul__RRR_RR(kk_sm,
...
class Benchmark(BenchmarkBase):
    def __init__(self, sm_nr:int, experiment_nr:int=0):
        super().__init__(sm_nr, 
                         name='fmul',
                         purpose="\n".join([
                            "This benchmark tests FMUL and measures how many cycles are required.",
                            "",
                            "FMUL takes 5 cycles."]),
        ...
        if experiment_nr in (0,1):
            ...
            offset = 1
            bInstrList = [BInstr(self.kk_sm, props, w) for w in range(offset, 15)]
