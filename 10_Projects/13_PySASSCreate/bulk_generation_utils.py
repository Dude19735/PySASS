...
class GenerationUtils:
    @staticmethod
    def gen_class(kk_sm:KK_SM, class_nr:int, total_class_nr:int, class_name:str, limit:int) -> typing.Tuple[typing.Dict, typing.List[set], typing.List[dict], int]:
        ...
        for f,tt in tt_funcs.items():
            if f in enc_vals:
                ...
                if const_bank_address and f == cb_bank:
                    ...
                    # Assume: these are generally floating point immediate values. They are already random, so maybe,
                    # don't touch it too much...
                    # NOTE: we iterate over the **fixed** values, not the other ones. For every fixed tuple, it will
                    # pick a fitting, random number for all of these.
                    pass
                else:
        ...
        domains = {e:tt.value.get_domain({}, filter_invalid=True) for e,tt in props.tt_exts.items()}
        ...
        for i in kk_sm.sass.encdom.fixed_iter2(instr_class=class_name, ankers=domains):
            enc_vals:dict = i

            if has_rd:
                enc_vals[props.cash_alias__rd] = barriers[props.cash_alias__rd]
            if has_wr:
                enc_vals[props.cash_alias__wr] = barriers[props.cash_alias__wr]
            # else: raise Exception(sp.CONST__ERROR_UNEXPECTED)
            enc_vals[props.cash_alias__usched_info] = SASS_Bits.from_int(TRANS2[-1], usched_info_ss.bit_len, usched_info_ss.signed)
            # if Instr_CuBin.check_expr_conditions(class_, enc_vals, throw=False):
            Params.append(enc_vals)
