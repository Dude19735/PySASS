"""
TODO: this file needs a new comment
...
class Generator:
    ...
    const int kernel_num = [[KernelCount]];
    ...
    std::map<std::string, void*> kernel_ptr = {[[KernelAddresses]]};
    ...
    for(const auto& ev : ordered_results){
        run_test(kernel_ptr[ev], input, loop_count, filename, index, ev, results[ev], ui_input_params);
        index++;
    }
    ...
    @staticmethod
    def gen_kernel(location:str, file_name:str, num_kernels:int, min_arch:int, max_arch:int, implicit_delete=False, instr_num:int=60):
...
if __name__ == '__main__':
    ...
    # NOTE: it is possible to compile with 3000 kernels if you split the compilation in two, i.e.
    # first compile one half of the SMs, then the other half.
    kernel_count = 1
    min_SM = 50
    max_SM = 120
    name = "template_{0}k_loop".format(kernel_count)
    
    location = os.path.dirname(os.path.realpath(__file__)) + "/template_projects/" + name
    filename = name
    
    if gg:
        Generator.gen_kernel(location, filename, kernel_count, min_SM, max_SM, implicit_delete=False, instr_num=60)
        exit(0)
