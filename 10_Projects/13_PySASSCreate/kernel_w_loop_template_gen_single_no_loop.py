...
class Generator:
    ...
    def gen_kernel(location:str, file_name:str, min_arch:int, max_arch:int, implicit_delete=False, instr_num:int=60):
        ...
        if not instr_num in (60, 120, 240): raise Exception("Only 60, 120 or 240 instructions are supported")
        ...
        if instr_num == 60:
            ...
            Kernels = [AllKernelShipPieces.KernelT_i240.replace(AllKernelShipPieces.ID, str(i)) for i in range(num_kernels)]
        else: raise Exception("Unsupported instruction number")
        RunTestNoLoop = AllKernelShipPieces.RunTestNoLoop
        ...
        cu_file = "{0}\n\n{1}\n\n{2}\n\n{3}\n\n{4}".format(Comment, Includes, "\n\n".join(Kernels), RunTestNoLoop, Main)
...
if __name__ == '__main__':
    gg = True
    name = 'template_1k_no_loop_240'
    location = os.path.dirname(os.path.realpath(__file__)) + "/template_projects/" + name
    filename = name
    
    if gg:
        Generator.gen_kernel(location, filename, 50, 120, implicit_delete=False, instr_num=240)
        exit(0)
